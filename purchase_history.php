...
<body>
    ...
    <div class="main_container">
        <h1 style="text-align: center; margin: 20px; padding: 2px;">Delivered Items</h1>
        <hr>
        <?php
        if (!empty($customerpurchases)) {
            foreach ($customerpurchases as $purchase):
                ...
                $delivered = $purchase['delivered_at'];

                // Show ONLY delivered items within last 3 days
                if ($status !== "delivered")
                    continue;

                // Make sure delivered date exists
                if (empty($delivered))
                    continue;

                // Check if delivered date is within last 3 days
                $deliveredTime = strtotime($delivered);
                $threeDaysAgo = strtotime("-3 days");

                if ($deliveredTime < $threeDaysAgo)
                    continue;
