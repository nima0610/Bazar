...
try {
    // Fetch purchase info including variant info
    $stmt = $db->prepare("SELECT product_id, sold, variant_size, variant_color 
                          FROM purchase_history 
                          WHERE id = :id AND user_id = :customer");
    ...
    if ($variantSize || $variantColor) {
        ...
        $updateStmt = $db->prepare("UPDATE product_variants 
                                    SET stock = stock + :qty 
        ...
        $updateStmt->bindValue(':size', $variantSize, PDO::PARAM_STR);
        $updateStmt->bindValue(':color', $variantColor, PDO::PARAM_STR);
        $updateStmt->execute();
    } else {
        // Update main product
        $updateStmt = $db->prepare("UPDATE product 
                                    SET product_stock = product_stock + :qty, sold = sold - :qty 
